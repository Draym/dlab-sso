...
export default class PlanningAssistantService {
    ...
    public async createBatch(serviceUuid: string, type: SessionType, from: Date, to: Date, startTime: TimeRequest, endTime: TimeRequest, dayPicker: DayPickerRequest, ignoreConflicts: boolean): Promise<PlanningSession[]> {
        ...
        for (const session of sessions) {
            try {
                const result = await planningSessionService.create(serviceUuid, session.type, session.start, session.end)
