...
const Errors = {
    ...
    NOT_FOUND_PlanningSession: () => new HttpException(ErrorCode.NOT_FOUND_PlanningSession, `Planning Session not found.`),
    ...
    NOT_FOUND_NewsletterSubscription: (userIdentifier: string) => new HttpException(ErrorCode.NOT_FOUND_NewsletterSubscription, `Newsletter subscription not found for user(${userIdentifier}).`),
    NOT_FOUND_DiscordValidator: (discordId: string) => new HttpException(ErrorCode.NOT_FOUND_DiscordValidator, `DiscordValidator not found for discordId[${discordId}].`),
    NOT_FOUND_DiscordAccount: (discordId: string) => new HttpException(ErrorCode.NOT_FOUND_DiscordAccount, `DiscordAccount not found for discordId[${discordId}].`),
    ...
    CONFLICT_WalletBind: (email: string) => new HttpException(ErrorCode.CONFLICT_WalletBind, `A wallet is already bound to the account (${email}).`),
    ...
    REQUIRE_DiscordEmail: () => new HttpException(ErrorCode.REQUIRE_DiscordEmail, `Discord email scope authentication is require to create a new Delysium account.`),
    ...
    REJECTED_WalletUnbind: (ownerIdentifier: string, address: string) => new HttpException(ErrorCode.REJECTED_WalletUnbind, `Wallet of address[${address}] is not bound to owner[${ownerIdentifier}] account.`),
    ...
    CONFLICT_DiscordBind: (ownerIdentifier: string) => new HttpException(ErrorCode.CONFLICT_DiscordBind, `A Discord account is already bound to the account [${ownerIdentifier}].`),
}
